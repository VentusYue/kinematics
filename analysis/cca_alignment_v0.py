...
def canoncorr(X0: np.array, Y0: np.array, fullReturn: bool = False) -> np.array:
    ...
    #
    # In theory, U == X @ A_full and V == Y @ B_full. In practice, for rank-deficient
    # cases or numeric issues, the most robust construction is via the orthonormal
    # bases used for the SVD:
    #   U_direct = Q1 @ L
    #   V_direct = Q2 @ M
    # which guarantees corr(U_k, V_k) == r_k (up to sign).
    ...
    # If the two disagree noticeably, prefer the direct construction.
    # (We check relative error on norms to avoid false positives from scale.)
