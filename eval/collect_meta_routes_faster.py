...
def run_batch_collection_v2(envs, actor_critic, batch_seeds, args, device, fast_inf, num_processes):
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            if should_finalize:
                # Check success (valid in adapt too)
...
def main():
    ...
    # CKPT setup (omitted for brevity, assume similar to fast.py but minimal)
    # To keep this file concise and focused on the optimization, I'll copy the minimal necessary parts.
    use_ckpt = args.ckpt_dir is not None
    ckpt_writer, manifest = None, None
    if use_ckpt and CKPT_AVAILABLE:
        ...
        if not manifest:
            ...
            manifest.num_tasks_target = args.num_tasks
            manifest.save(args.ckpt_dir)
        ckpt_writer = RoutesShardWriter(args.ckpt_dir, args.ckpt_shard_size, args.ckpt_flush_secs, manifest)
