...
def train(args, seeds):
    ...
    # Give DQN runs a unique default checkpoint path to avoid clashes with PPO.
    ...
    if getattr(args, "model_path", "models/model.tar") == "models/model.tar":
        args.model_path = os.path.join("models", f"{name}.tar")
    ...
    settings = wandb.Settings(mode="online" if args.wandb else "offline")
    wandb.init(
        project=getattr(args, "wandb_project", "dqn_procgen"),
        name=name,
        config=vars(args),
        tags=["dqn", "procgen"] + (args.wandb_tags.split(",") if getattr(args, "wandb_tags", "") else []),
        group=(args.wandb_group if getattr(args, "wandb_group", "") else None),
        settings=settings,
    )
    ...
    for t in range(num_steps):
        ...
        if t >= args.start_timesteps and t % args.eval_freq == 0:
            ...
            if args.record_eval_video and t % args.eval_video_freq == 0:
                env_names = args.eval_video_envs.split(",")
                eval_and_record(args, agent, env_names, args.eval_video_episodes, t * args.num_processes)
        ...
        if (not getattr(args, "disable_checkpoint", False)) and getattr(args, "save_interval", 0) > 0:
            ...
            if elapsed_minutes >= args.save_interval:
                total_num_steps = t * args.num_processes
...
def eval_and_record(args, agent, env_names, num_episodes, step):
