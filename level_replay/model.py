...
class ResNetBase(NNBase):
    ...
    def __init__(self, num_inputs, recurrent=False, hidden_size=256, channels=(16, 32, 32)):
        # When recurrent, the GRU operates on the post-conv, post-FC feature
        # embedding of size `hidden_size`, not on raw input channels.
...
class SmallNetBase(NNBase):
    """
    Small convolutional network used for Procgen.
    """

    def __init__(self, num_inputs, recurrent=False, hidden_size=256):
        # When recurrent, the GRU operates on the post-conv, post-FC feature
        # embedding of size `hidden_size`, not on raw input channels.
        super(SmallNetBase, self).__init__(recurrent, hidden_size, hidden_size)

        # Use `num_inputs` for the first conv in-channel to support arbitrary
        # numbers of input channels (e.g. 3 for RGB).
        self.conv1 = Conv2d_tf(num_inputs, 16, kernel_size=8, stride=4)
