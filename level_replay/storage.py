...
class RolloutStorage(object):
    def __init__(
        ...
        self.episode_dones = torch.zeros(num_steps, num_processes, 1)
        self.trial_ids = torch.zeros(num_steps, num_processes, 1, dtype=torch.int)
    ...
    def to(self, device):
        ...
        self.episode_dones = self.episode_dones.to(device)
        self.trial_ids = self.trial_ids.to(device)
    ...
    def insert(
        ...
        if episode_dones is not None:
            self.episode_dones[self.step].copy_(episode_dones)
        if trial_ids is not None:
            self.trial_ids[self.step].copy_(trial_ids)
